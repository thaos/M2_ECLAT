...
Spatialement, nous aggrégeons les données pour des patchs de differentes tailles (1x1, 7x5, 11x9 points de grille) centrés autours du point de grille situé à 2.5 degrés de longitude et 47.68 degrés de latitude. L'aggrégation est realisée soit en prenant la moyenne ou le maximum sur l'ensembre des points de grille du patch.

Temporellement, les données à aggréger consistent soit à la journée du 15 juillet, soit au mois de juillet dans son ensemble, soit à l'ensemble de l'année. L'aggrégation est realiser soit en prenant la moyenne ou le maximum sur l'ensembre des pas de temps de la periode d'intérêt.
...
Les distributions peuvent être représentées sous forme d'histogramme (fonction *hist*), de densité estimée par des méthodes à noyau (fonction *density*) ou par fonction de répartition empirique (fonction *ecdf*). 
...
```{r}
# Choisir le type de série temporelles: variable/agrégation spatial/agrégation temporelle
y <- tas_smean_tmean
ylim <- range(y)
par(mfrow = c(3, 3))
for(i in 1:3){
...
- Calculer la matrice des corrélation entre les differents points de grille du jeu de donnée initial et représenter sous forme de carte la corrélation d'un point de grille avec tous les autres.
...
```
- Calculer les correlations temporelles pour chaque point de grille du jeu de donnée initial via la fonction *acf* de R. 
