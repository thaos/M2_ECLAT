...
la variable *anomalie* est une matrice de même dimension que la matrice *tas_df*.
```{r}
anomalies <- 
...
```
Comment le cycle saisonnier estimer de cette façon vous parait-il ?
...
```{r}
# Exemple: On défini la distance entre deux modèles par la distance euclidienne entre leurs séries temporelles brutes.
...
### Reprentation des modèles en fonction de leur distance respective avec les uns et les autres sur un plan 2d
```{r}
