...
Le cours n'est pas exhaustif et ne fait que survoler certains aspects. L'objectif est de vous familiariser avec certaines notions importante et de vous orienter vers des ressources complémentaires pous vous renseigner d'avantage de votre côté.
...
Bien sur, l'objectif de ce cours est aussi d'en discuter ensemble.

C'est la premiere année que se tient ce cours (et dans un contexte un peu particulier...). Toutes les remarques dans le but de l'améliorer sont les bienvenus. En particulier, ne pas hésiter poser des questions. 
...
- Séance 4: Séances pratique en R sur les effets de l'aggrégation spatiale et l'effet de la varibilité interne sur les estimations statistiques.

- Séance 5: Introduction au downscaling statistique et corréction de bias par la pratique.

Il y a beaucoup de pratique avec le logiciel R. Il vous faut donc l'installer ainsi que R studio.
...
***Le climat est comme une distribution de probabilité et la méteo  ses réalisation.***
...
Lorsque l'on simule l'évolution du climat au 20ème, on ne connait pas assez précisément les conditions initiales de l'ère pré-industrielle. 
...
Par exemple, on peut modéliser des données horraires par:
$$
\begin{aligned}
...
<br/>
- **Dependance temporelle** (autocorrelation)
...
<br/>
- **Stationnaire**: Les propriétés statistique du bruit évoluent elle dans le temps ?
...
<br/>
Quelle est l'echantillonnage spatiale et temporel ?
...
Où se situe les simulations d'un modèle par rapport aux autres?
...
Pour simuler un système chaotique, la connaissance des condition initiales est essentielle, d'où l'impornance de l'assimilation pour la prévision du temps.
...
La physique est nécessaire pour la modélistion des opérateur $M$ et $K$.
...
- Attention aux bias du modèle et des observations
