...
- Séance 4: Séances pratique en R sur les effets de l’agrégation spatiale et l'effet de la variabilité interne sur les estimations statistiques.
...
Cette séances reprend la trame et beaucoup d'éléments du chapitre 2 de
<p align="center">
...
Les autres sources sont citées au fur et à mesures des slides.
...
- Introduire quelques outils de base pour commencer/essayer de répondre à ces questions.

- Présentation non-exhaustive et superficielle.

- A vous de creuser d'avantage les sujets etde nous questionner d'avantage si besoin.
...
**Questions** Calculer l'espérance de $\overline{X}_n$ ? $\widehat F_n(x)$ converge-t-il vers  $F(x)$ ?
---
# Variance
...
Soit $X_n$ une suite de variables aléatoires réelles indépendantes et identiquement distribuées (i.i.d.),  la variance $\sigma^2$ de cette loi peut être estimée à l’aide de la variance empirique
...
&\Leftrightarrow \mathbb{P}(X_1 \leq x_T) = 1 - \frac{1}{T}
\end{align}$$
...
---
# Covariance: quelques propriétés
...
\end{align}$$
où $X$, $Y$, $W$  et $V$ sont des variables aléatoires réelles et $a$, $b$, $c$ et  $d$ sont des constantes réelles.
...
.footnote[
source Denis Boigelot](https://commons.wikimedia.org/wiki/File:Correlation_examples2.svg).
]
...
```{r ic_n01, echo=FALSE, fig.height=6, dev='svg'}
...
ypol <- c(0, dnorm(x[x < -xstar]), dnorm(-xstar), 0)
polygon(xpol, ypol, col = "pink", border = NA)
xpol <- c(5, rev(x[x > xstar]), rep(xstar, 2))
ypol <- c(0, rev(dnorm(x[x > xstar])), dnorm(xstar), 0)
polygon(xpol, ypol, col = "pink", border = NA)
lines(x, dnorm(x))
segments(
  x0 = -xstar,
...
text(
  x = c(-xstar-0.3, -xstar+0.4),
  y = c(0.02, -0.015),
  labels = c(expression(paste(alpha,"/2")),  expression("-z"[1-alpha/2])),
  col = "red"
)
segments(
  x0 = xstar,
...
2. Calculer le maximum de cette échantillon.

3. Répéter la même opération B=300 fois pour avoir un échantillon de B maxima.
...
Pour la non-stationnarité, une solution est de travailler sur des sous-périodes que l'on peut considérer comme stationnaires.

On peut aussi essayer d'intégrer la non-stationnarité dans la modélisation.
---
# Covariables dans les parametres

On suppose les variables aléatoires réelles, $Y_t, t=  1,\ldots, T$, indépendantes et appartenant à la même famille de loi mais avec  des valeurs de paramètres différentes.
...
- On simplifie souvent le problème pour faciliter la modélisation : .e.g. hypothèse d'indépendance.

- On se base sur des propriétés asymptotiques: attention à la vitesse de convergence.

- Attention à l'extrapolation des modèles avec covariables: e.g. ce n'est pas parce qu'on observe une tendance (linéaire) qu'elle se poursuivra dans le futur.
