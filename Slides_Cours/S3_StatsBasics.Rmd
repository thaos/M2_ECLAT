...
Soit $X_n$ une suite de variables aléatoires réelles indépendantes et identiquement distribuées(iid) admettant une espérance $\mu = \mathbb{E}[X]$. La moyenne empirique 
...
# Correlation et independance
La valeur du coéfficient de corrélation est comprise entre  −1 et  +1. Elle indique le degré de dependance linéaire entre deux variable.  Un coefficient de corrélation de 1 correspond à une parfaite relation linéaire croissante , −1 correspond à une parfaite relation inverse décroissante. Lorsqu'il est proche de zéro, cela indique une absence de relation linéaire entre les variables.
...
$$
---
# Coefficient de corrélation de Pearson: exemples
...
ou de manière équivalente, si les densités de probabilité  $f_X(x)$ and $f_Y(y)$ et la densité de probabilité jointe $f_{X,Y}(x,y)$ existent,
